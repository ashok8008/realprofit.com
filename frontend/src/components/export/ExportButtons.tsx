...
export function ExportToPDFButton({ elementId, title }: ExportProps) {
  ...
  const handleExport = async () => {
    try {
      ...
      doc.setFontSize(9);
      doc.text("Generated by RealProfits.com", 10, 22);
      ...
      try {
        ...
        const maxPerPage = pageHeight - 20;

        if (imgHeight <= maxPerPage) {
          doc.addImage(imgData, "PNG", 10, y, imgWidth, imgHeight);
        } else {
          const totalPages = Math.ceil(imgHeight / maxPerPage);
          for (let p = 0; p < totalPages; p++) {
            if (p > 0) {
              doc.addPage();
              y = 10;
            }
            const sliceHeight = Math.min(maxPerPage, remainingHeight);
            doc.addImage(imgData, "PNG", 10, y - (p * maxPerPage), imgWidth, imgHeight);
