    ...
    <div className="space-y-8">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="space-y-2">
          ...
          <Input type="number" step="0.1" value={saveRate} onChange={e => setSaveRate(Number(e.target.value) || 0)} />
        </div>
        <div className="space-y-2">
          <Label>Investment Expected Return (%)</Label>
          <Input type="number" step="0.1" value={investRate} onChange={e => setInvestRate(Number(e.target.value) || 0)} />
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-muted/30 p-4 rounded-xl border text-center">
          ...
          <div className="text-2xl font-bold mt-1">${finalSave.toLocaleString()}</div>
        </div>
        <div className="bg-muted/30 p-4 rounded-xl border text-center">
          <h4 className="text-sm font-medium text-muted-foreground">Market Investments</h4>
          <div className="text-2xl font-bold mt-1 text-primary">${finalInvest.toLocaleString()}</div>
        </div>
        <div className="bg-primary/10 p-4 rounded-xl border border-primary/20 text-center">
          ...
          <div className="text-2xl font-bold mt-1 text-primary">${difference.toLocaleString()}</div>
        </div>
      </div>
